...
这里是一个 intersection type，表示这个函数“同时”是两种类型。我们先看第一行，它表示： route 接受一个 Flask  对象 (self) 和一个 str，然后生成一个中间函数。这个函数接受另一个函数（index），然后又生成一个函数。这个函数被调用之后，会返回一个 str。其中 index 函数的类型是 () -> str。这就是为什么你可以这样调用它：
