...
这是说第一个参数 `template_name_or_list` 的类型或者是 str 或者是 [str] （含有str 的 list）。如果你注意一下这个参数的英文含义 "template name or list"，就会觉得仿佛 PySonar2 能读懂英语一样。然而它其实不会英语，它只会 Python。它从代码之间的调用关系，通过异常强大的类型推导，找到了这个参数的类型。
