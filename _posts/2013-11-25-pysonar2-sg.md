...
最有意思的是那个 `render_template`。PySonar2 为它推导出来的类型是：

	templating.render_template(template_name_or_list, **context)
	str -> ?
	| [str] -> ?

这是说，第一个参数 `template_name_or_list` 的类型或者是 str 或者是 [str] （含有str 的 list）。如果你注意一下这个参数的英文含义 "template name or list"，就会觉得仿佛 PySonar2 能读懂英语一样。然而 PySonar2 其实不会英语，它只会 Python。它通过代码之间的调用关系和异常强大的类型推导，找到了这个参数的类型。
...
方法很简单：把你的 GitHub 地址去掉 `http://`之后放到 `http://sourcegraph.com/` 后面，然后 Sourcegraph 就会显示一个等待页面，同时自动开始分析这个 repo，一般大小的代码库几分钟到半个小时就会处理完毕。目前支持的语言是 Go, JavaScript, Python, Ruby（更多的语言会陆续加入）。
...
<a href="http://github.com/yinwang0/psydiff"><img src="http://www.yinwang.org/resources/psydiff-badge.png" width="40%"></a>

它表示 psydiff 的代码被看过的次数。你也可以使用其他的一些徽章，比如最常用的函数，交叉引用数，用户数，等等：

<a href="https://sourcegraph.com/github.com/yinwang0/psydiff/badges"><img src="http://www.yinwang.org/resources/sg-badges.png"  width="25%"></a>
